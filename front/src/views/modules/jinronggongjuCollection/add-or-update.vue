    ...
    <div class="addEdit-block">
        <el-form
            ...
            <input id="yonghuId" name="yonghuId" type="hidden">
                <el-col :span="12">
                    <el-form-item class="select" v-if="type!='info'"  label="类型" prop="jinronggongjuCollectionTypes">
                        <el-select v-model="ruleForm.jinronggongjuCollectionTypes" :disabled="ro.jinronggongjuCollectionTypes" placeholder="请选择类型">
                            <el-option
                                v-for="(item,index) in jinronggongjuCollectionTypesOptions"
                    ...
                    <div v-else-if="type=='info'">
                        <el-form-item class="input" label="类型" prop="jinronggongjuCollectionValue">
                        <el-input v-model="ruleForm.jinronggongjuCollectionValue"
...
<script>
    ...
    export default {
        data() {
            return {
                ...
                ro:{
                    ...
                    yonghuId: false,
                    jinronggongjuCollectionTypes: false,
                    insertTime: false,
                },
                ruleForm: {
                    jinronggongjuId: '',
                    yonghuId: '',
                    jinronggongjuCollectionTypes: '',
                    insertTime: '',
                },
                jinronggongjuCollectionTypesOptions : [],
        ...
        created() {
            ...
            //获取下拉框信息
                this.$http({
                    url:`dictionary/page?page=1&limit=100&sort=&order=&dicCode=jinronggongju_collection_types`,
                    method: "get"
                }).then(({ data }) => {
                    if (data && data.code === 0) {
                        this.jinronggongjuCollectionTypesOptions = data.data.list;
                    }
                });
        ...
        methods: {
            ...
            info(id) {
                let _this =this;
                _this.$http({
                    url: `jinronggongjuCollection/info/${id}`,
                    method: 'get'
                }).then(({ data }) => {
                    if (data && data.code === 0) {
            ...
            onSubmit() {
                this.$refs["ruleForm"].validate(valid => {
                    if (valid) {
                        this.$http({
                            url:`jinronggongjuCollection/${!this.ruleForm.id ? "save" : "update"}`,
                            method: "post",
                            data: this.ruleForm
                        }).then(({ data }) => {
                            if (data && data.code === 0) {
                                this.$message({
                                    ...
                                    onClose: () => {
                                        ...
                                        this.parent.jinronggongjuCollectionCrossAddOrUpdateFlag = false;
                                        this.parent.search();
