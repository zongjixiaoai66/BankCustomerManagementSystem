    ...
    <div class="addEdit-block">
        <el-form
            ...
            <input id="yinhangkaId" name="yinhangkaId" type="hidden">
               <el-col :span="12">
                   <el-form-item class="input" v-if="type!='info'"  label="记录卡号" prop="yinhangkaJiluUuidNumber">
                       <el-input v-model="ruleForm.yinhangkaJiluUuidNumber"
                                 placeholder="记录卡号" clearable  :readonly="ro.yinhangkaJiluUuidNumber"></el-input>
                   </el-form-item>
                   <div v-else-if="type=='info'">
                       <el-form-item class="input" label="记录卡号" prop="yinhangkaJiluUuidNumber">
                           <el-input v-model="ruleForm.yinhangkaJiluUuidNumber"
                                     placeholder="记录卡号" readonly></el-input>
                       </el-form-item>
                   </div>
               </el-col>
                <el-col :span="12">
                    <el-form-item class="select" v-if="type!='info'"  label="银行卡记录类型" prop="yinhangkaJiluTypes">
                        <el-select v-model="ruleForm.yinhangkaJiluTypes" :disabled="ro.yinhangkaJiluTypes" placeholder="请选择银行卡记录类型">
                            <el-option
                                v-for="(item,index) in yinhangkaJiluTypesOptions"
                    ...
                    <div v-else-if="type=='info'">
                        <el-form-item class="input" label="银行卡记录类型" prop="yinhangkaJiluValue">
                        <el-input v-model="ruleForm.yinhangkaJiluValue"
                            placeholder="银行卡记录类型" readonly></el-input>
                        </el-form-item>
                    </div>
                </el-col>
               <el-col :span="12">
                   <el-form-item class="input" v-if="type!='info'"  label="记录金额" prop="yinhangkaJiluJine">
                       <el-input v-model="ruleForm.yinhangkaJiluJine"
                                 placeholder="记录金额" clearable  :readonly="ro.yinhangkaJiluJine"></el-input>
                   </el-form-item>
                   <div v-else-if="type=='info'">
                       <el-form-item class="input" label="记录金额" prop="yinhangkaJiluJine">
                           <el-input v-model="ruleForm.yinhangkaJiluJine"
                                     placeholder="记录金额" readonly></el-input>
                       </el-form-item>
                   </div>
               </el-col>
                <el-col :span="24">
                    <el-form-item v-if="type!='info' && !ro.yinhangkaJiluContent"  label="详情" prop="yinhangkaJiluContent">
                        <editor style="min-width: 200px; max-width: 600px;"
                                v-model="ruleForm.yinhangkaJiluContent"
                                class="editor"
                                action="file/upload">
                        </editor>
                    </el-form-item>
                    <div v-else-if="type=='info'">
                        <el-form-item v-if="ruleForm.yinhangkaJiluContent" label="详情" prop="yinhangkaJiluContent">
                            <span v-html="ruleForm.yinhangkaJiluContent"></span>
                        </el-form-item>
                    </div>
                </el-col>
            </el-row>
    ...
    export default {
        data() {
            return {
                ...
                ro:{
                    yinhangkaId: false,
                    yinhangkaJiluUuidNumber: true,
                    yinhangkaJiluTypes: false,
                    yinhangkaJiluJine: false,
                    yinhangkaJiluContent: false,
                    insertTime: false,
                },
                ruleForm: {
                    yinhangkaId: '',
                    yinhangkaJiluUuidNumber: new Date().getTime(),
                    yinhangkaJiluTypes: '',
                    yinhangkaJiluJine: '',
                    yinhangkaJiluContent: '',
                    insertTime: '',
                },
                yinhangkaJiluTypesOptions : [],
                yinhangkaOptions : [],
                rules: {
                   yinhangkaId: [
                   ...
                   yinhangkaJiluUuidNumber: [
                              { required: true, message: '记录卡号不能为空', trigger: 'blur' },
                          ],
                   yinhangkaJiluTypes: [
                              { required: true, message: '银行卡记录类型不能为空', trigger: 'blur' },
                              {  pattern: /^[0-9]*$/,
                                  message: '只允许输入整数',
                                  trigger: 'blur'
                              }
                   ...
                   yinhangkaJiluJine: [
                              { required: true, message: '记录金额不能为空', trigger: 'blur' },
        ...
        created() {
            ...
            //获取下拉框信息
                this.$http({
                    url:`dictionary/page?page=1&limit=100&sort=&order=&dicCode=yinhangka_jilu_types`,
                    method: "get"
                }).then(({ data }) => {
                    if (data && data.code === 0) {
                        this.yinhangkaJiluTypesOptions = data.data.list;
                    }
                });
        ...
        methods: {
            ...
            info(id) {
                let _this =this;
                _this.$http({
                    url: `yinhangkaJilu/info/${id}`,
                    method: 'get'
                }).then(({ data }) => {
                    if (data && data.code === 0) {
                        _this.ruleForm = data.data;
                        _this.yinhangkaChange(data.data.yinhangkaId)
                        _this.ruleForm.yinhangkaJiluContent = _this.ruleForm.yinhangkaJiluContent.replaceAll("src=\"upload/","src=\""+this.$base.url+"upload/");
                    } else {
            ...
            onSubmit() {
                this.$refs["ruleForm"].validate(valid => {
                    if (valid) {
                        this.ruleForm.yinhangkaJiluContent = this.ruleForm.yinhangkaJiluContent.replaceAll(this.$base.url,"");
                        this.$http({
                            url:`yinhangkaJilu/${!this.ruleForm.id ? "save" : "update"}`,
                            method: "post",
                            data: this.ruleForm
                        }).then(({ data }) => {
                            if (data && data.code === 0) {
                                this.$message({
                                    ...
                                    onClose: () => {
                                        ...
                                        this.parent.yinhangkaJiluCrossAddOrUpdateFlag = false;
                                        this.parent.search();
